...
impl SQLiteDatabase {
    pub async fn new(db_url: &str) -> Self {
        let mut connection_options = SqliteConnectOptions::from_str(db_url).expect("Unable to create connection options.");
...
impl Database for SQLiteDatabase {
    async fn get_player_info(&self, player_uid: &str) -> Result<PlayerInfo, sqlx::Error> {
        sqlx::query_as::<_, PlayerInfo>(queries::sqlite::GET_PLAYER_INFO)
            .bind(player_uid)
            .fetch_one(&self.pool)
            .await
    }
    ...
    async fn insert_team_scores(&self, game_id: i64, scores: &Vec<i64>) -> Result<(), ()> {
        let _res = sqlx::query(queries::sqlite::INSERT_TEAM_SCORES)
            .bind(game_id)
            .bind(scores[0])
            .bind(scores[1])
            .bind(scores[2])
    ...
    async fn insert_game_and_get_id(&self, game_version: &str, server_name: &str, server_ip: &str, server_port: &str, host_player: &str, game: &Game) -> Result<i64, ()> {
        let res: (i64,) = sqlx::query_as(queries::sqlite::INSERT_GAME_AND_GET_ID)
            .bind(game_version)
            .bind(server_name)
            .bind(server_ip)
            .bind(server_port)
    ...
    async fn insert_player(&self, uid: &str) -> Result<String, ()> {
        let _res = sqlx::query(queries::sqlite::INSERT_PLAYER)
            .bind(uid)
            .execute(&self.pool)
            .await.map_err(|_| ())?;
    ...
    async fn insert_player_info(&self, player: &Player) -> Result<(), ()> {
        sqlx::query(queries::sqlite::INSERT_PLAYER_INFO)
            .bind(&player.uid)
            .bind(&player.ip)
    ...
    async fn insert_game_player_result(&self, game_id: i64, player: &Player, exp: u32) -> Result<(), ()> {
        sqlx::query(queries::sqlite::INSERT_GAME_PLAYER_RESULT)
            .bind(game_id)
            .bind(&player.uid)
            .bind(&player.team)
